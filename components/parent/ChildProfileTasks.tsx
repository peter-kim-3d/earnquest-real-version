        ...
        <div>
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-text-main dark:text-white flex items-center gap-2">
                    <AppIcon name="task_alt" className="text-primary h-6 w-6" aria-hidden="true" />
                    {t('title')}
                </h2>
            ...
            {tasks && tasks.length > 0 ? (
                        ...
                        <div key={task.id} className={`rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm group relative flex flex-col ${task.isDisabled ? 'bg-gray-50' : ''}`}>
                            <div className="p-4 flex-1">
                                ...
                                <div className="flex items-start justify-between gap-3 mb-3">
                                    <div className={`h-12 w-12 rounded-lg flex items-center justify-center flex-shrink-0 ${task.isDisabled ? 'bg-gray-100 grayscale opacity-50' : 'bg-primary/10'}`} aria-hidden="true">
                                        <AppIcon
                                            name={task.icon || 'task'}
                                    ...
                                    <div className="flex-shrink-0">
                                        {task.frequency === 'one_time' ? (
                                            <button
                                                ...
                                                className="h-10 w-10 flex items-center justify-center rounded-lg bg-white/80 dark:bg-gray-800/80 text-red-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-all hover:bg-red-50 dark:hover:bg-red-900/30 shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:opacity-100"
                ...
                <div className="text-center py-12 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                    <div className="h-16 w-16 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center mx-auto mb-4">
                        <AppIcon name="task" className="text-gray-400 dark:text-gray-600 h-8 w-8" size={32} aria-hidden="true" />
                    </div>
