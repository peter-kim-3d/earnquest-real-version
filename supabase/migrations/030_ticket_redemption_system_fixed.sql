-- ============================================
-- Ticket Redemption System Migration (FIXED)
-- ============================================
...
-- 3. DROP OLD STATUS CONSTRAINT FIRST (before updating values)
ALTER TABLE reward_purchases
...
-- 4. NOW migrate existing status values (constraint is gone)
...
-- 5. ADD NEW STATUS CONSTRAINT (after migration complete)
ALTER TABLE reward_purchases
ADD CONSTRAINT reward_purchases_status_check
CHECK (status IN ('active', 'use_requested', 'used', 'cancelled'));

-- 6. Create index for guardrail query optimization
...
-- 7. Update purchase_reward function for instant completion logic
...
-- 8. Function: request_ticket_use (Child requests to use a screen reward)
...
-- 9. Function: approve_ticket_use (Parent approves screen reward use)
...
-- 10. Function: fulfill_ticket (Parent marks item/experience as given)
