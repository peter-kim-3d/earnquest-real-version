-- Fix v_child_today_tasks view to use family's timezone for day-of-week calculation
-- Bug: EXTRACT(DOW FROM NOW()) uses UTC, but families may be in different timezones
-- Example: Monday 10 AM in Seoul = Sunday 1 AM UTC, so weekly Monday tasks don't show
...
SELECT
  ...
  (
    ...
    WHERE tc.task_id = t.id
      ...
      -- Use family timezone for "today" calculation
      AND tc.requested_at >= date_trunc('day', NOW() AT TIME ZONE COALESCE(f.settings->>'timezone', 'UTC'))
